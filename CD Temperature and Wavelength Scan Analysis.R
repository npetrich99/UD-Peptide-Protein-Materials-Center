...
# blank file must be edited, starting at line 162. When running the code, users 
...
# ensure that the values are those that were measured in the CD data.
...
server <- function(input, output, session) {
  ...
  # Reactive Values for Starting and Ending Wavelengths:
  ...
  observeEvent(input$submit, {
    for (i in 1:num_cells) {
      ...
      assign(paste0("C", i, "C"), values$C_C, envir = .GlobalEnv)
      assign(paste0("C", i, "P"), values$C_P, envir = .GlobalEnv)
      assign(paste0("C", i, "R"), values$C_R, envir = .GlobalEnv)
    }
    
    # Store Starting and Ending Wavelengths:
    wavelengths$starting <- input$starting_wavelength
    wavelengths$ending <- input$ending_wavelength
  ...
  process_cell_f <- function(cell_number) {
    ...
    if (length(cd_file) > 0) {
      ...
      theme_update(
        ...
        axis.ticks.length = unit(c(-0.15, -0.1),
                            "inches"), 
      ...
      p <- p +
        ...
        scale_x_continuous(expand = c(0.0090, 0),
                           breaks = function(x) seq(190, 250, by = 5),
                           minor_breaks = NULL,
                           labels = function(x) ifelse(x %% 10 == 0,
                                    as.character(x), ""), 
                                    # Keep Labels for Major Ticks Only
                           limits = c(190, 250)) + # Set X-Axis Limits
        scale_y_continuous(expand = c(0.0045, 0),
                           breaks = function(y) seq(-30, 55, by = 5),
                           minor_breaks = function(y) seq(-30, 55, by = 5),
                                          # Set Minor Tick Positions for Y-Axis
                           labels = function(y) ifelse(y %% 10 == 0,
                                    as.character(y), ""), 
                                    # Keep Labels for Major Ticks Only
                           limits = c(-30, 55)) + # Set Y-Axis Limits
  ...
  process_melt_f <- function(cell_number, directory) {
    ...
    if (length(melt_file) > 0) {
      ...
      p <- p +
        ...
        scale_x_continuous(expand = c(0.0090, 0),
                           breaks = function(x) seq(10, 90, by = 5),
                           minor_breaks = NULL,
                           labels = function(x) ifelse(x %% 10 == 0,
                                                       as.character(x), ""), 
                           # Keep Labels for Major Ticks Only
                           limits = c(10, 90)) + # Set X-Axis Limits
        scale_y_continuous(expand = c(0.0045, 0),
                           breaks = function(y) seq(-25, 0, by = 2.5),
                           minor_breaks = function(y) seq(-25, 0, by = 0.5),
                           # Set Minor Tick Positions for Y-Axis
                           labels = function(y) ifelse(y %% 5 == 0,
                                                       as.character(y), ""), 
                           # Keep Labels for Major Ticks Only
                           limits = c(-25, 0)) # Set Y-Axis Limits
...
################################################################################
  ...
  melting_combined <- function(directory) {
    ...
    initial_shapes <- c(21, 22, 23, 24, 25, 20)
    ...
    theme_update(
      ...
      legend.text = element_text(size = 10), # Adjust Legend Text Size
      legend.position = c(0.825, 0.25) # Position Legend
    )
    ...
    p <- ggplot() +
      ...
      scale_x_continuous(expand = c(0.0085, 0),
      ...
      scale_y_continuous(expand = c(0.007, 0),
                         breaks = seq(-25, 0, by = 2.5),
                         minor_breaks = seq(-25, 0, by = 0.5),  
                         labels = function(y) ifelse(y %% 5 == 0,
                                                     as.character(y), ""),  
                         limits = c(-25, 0))
    ...
    for (i in 1:6) {
      ...
      if (temperature_col %in% colnames(combined_melting) && 
        ...
        p <- p + geom_point(data = series_data, aes_string(x = temperature_col,
                            y = cd_col, color = as.factor(actual_index),
                            fill = as.factor(actual_index),
                            shape = as.factor(actual_index)), size = 3)
    ...
    # Export Plot with Custom Width and Height:
    ggsave(filename = filename, plot = p, device = "svg", width = 4,
           height = 4)
  }
  ...
  melting_combined <- function(directory) {
    ...
    for (i in 1:6) {
      ...
      if (temperature_col %in% colnames(combined_melting) && 
        ...
        p <- p + geom_point(data = series_data, aes_string(x = temperature_col,
             y = cd_col, color = as.factor(actual_index),
             fill = as.factor(actual_index), shape = as.factor(actual_index)), 
             size = 3)
